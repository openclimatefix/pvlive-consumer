...
from nowcasting_datamodel.models.gsp import Location, LocationSQL, GSPYield

from gspconsumer.gsps import (
    filter_gsps_which_have_new_data,
    get_gsps
)
...
def test_filter_pv_systems_which_have_new_data(db_session):
    ...
    gsps = get_latest_gsp_yield(
        session=db_session, gsps=gsps, append_to_gsps=True
    )
